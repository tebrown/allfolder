...
rcmail_all ()
{
  if (rcmail.env.uid || rcmail.message_list
	...
	get_selection ().join (","), b = rcmail.set_busy (!0, "loading");
...
function
rcmail_all_contextmenu (a)
{
(rcmail.env.uid || rcmail.message_list
   && rcmail.message_list.get_selection ().length)
&& 0 < rcmail.message_list.get_selection ().length && rcmail_all (a)
}
...
{
  var a;
  if (rcmail.env.all_folder && rcmail.env.all_folder_icon
     && (a = rcmail.get_folder_li (rcmail.env.all_folder, "", !0)))
  ...
  /*var a;
  if (rcmail.env.all_folder && 
      rcmail.env.all_folder_icon && 
      (a = rcmail.get_folder_li (rcmail.env.all_folder, "", !0)))
        "larry" != rcmail.env.skin ? $ (a).css ("background-image", "url(" + rcmail.env.all_folder_icon + ")") : $ (a).addClass ("archive"),
    $ (a).insertAfter ("#mailboxlist.inbox"), 
    a=$ ("._all"), $ (a.get (0)).insertBefore ("#rcmContextMenu.drafts")*/

function rcmail_all_init ()
{
  window.rcm_contextmenu_register_command
      && rcm_contextmenu_register_command ("all",
					     "rcmail_all_contextmenu",
					     rcmail.
					     gettext
					     ("allfolder.buttontitle"),
					     "delete", null, !0)};
